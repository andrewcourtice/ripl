...
describe('formatTime', () => {
    ...
    test('Should format a date with a default short format', () => {
        const result = formatTime(new Date(2024, 0, 15), {
            locale: LOCALE,
        });
    ...
    test('Should accept an epoch millisecond value', () => {
        ...
        expect(formatTime(millis, {
            locale: LOCALE,
        })).toBe('Jan 15, 2024');
    });

    test('Should honour explicit field options', () => {
        const result = formatTime(new Date(2024, 5, 1), {
            locale: LOCALE,
            month: 'long',
            year: 'numeric',
        });
    ...
    test('Should format time-of-day fields', () => {
        const result = formatTime(new Date(2024, 0, 1, 9, 30, 0), {
            locale: LOCALE,
            hour: '2-digit',
            minute: '2-digit',
            hour12: false,
