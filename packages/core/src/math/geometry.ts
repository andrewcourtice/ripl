...
export function geometryDegreesToRadians(degrees: number): number {
    return degrees * Math.PI / 180;
}

/** Converts radians to degrees. */
export function geometryRadiansToDegrees(radians: number): number {
    return radians * 180 / Math.PI;
}

/** Tests whether two points have identical coordinates. */
export function geometryPointsEqual([x1, y1]: Point, [x2, y2]: Point): boolean {
    return x1 === x2 && y1 === y2;
}

/** Returns the midpoint between two points. */
export function geometryMidpoint(pointA: Point, pointB: Point): Point {
    return geometryWaypoint(pointA, pointB, 0.5);
}

/** Returns a point along the line segment between two points at the given normalised position (0–1). */
export function geometryWaypoint([x1, y1]: Point, [x2, y2]: Point, position: number): Point {
    return [
        x1 + (x2 - x1) * position,
        y1 + (y2 - y1) * position,
    ];
...
export function geometryEuclideanDistance(dp: number, dq: number): number {
    return Math.sqrt(dp ** 2 + dq ** 2);
}

/** Returns the point at a given angle and distance from an optional centre. */
export function geometryThetaPoint(angle: number, distance: number, cx?: number, cy?: number): Point {
    return [
...
export function geometryPolygonPoints(
    sides: number,
    cx: number,
    cy: number,
    radius: number,
    ...
    const points = Array.from({ length: sides }, (_, i) => {
        return geometryThetaPoint(i * angle - offset, radius, cx, cy);
    });
...
export function geometryContainingBox<TValue>(value: TValue[], identity: (value: TValue) => Box): Box {
    if (!value.length) {
        return Box.empty();
    }
...
export function geometryIsPointInBox([x, y]: Point, { left, top, bottom, right }: Box) {
    return x >= left
        && x <= right
        && y >= top
        && y <= bottom;
...
export function geometryNormaliseBorderRadius(borderRadius: number | BorderRadius): BorderRadius {
...
export function geometryPathLength(pathData: string): number {
...
export function geometrySamplePathPoint(pathData: string, distance: number): PathPoint {
    const pathEl = getRefPathElement();
