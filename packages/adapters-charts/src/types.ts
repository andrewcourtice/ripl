...
/**
 * A chart's options as component props, with any option whose name a UI framework reserves renamed.
 *
 * Only `key` is affected: both Vue and React consume a `key` prop as the element key, so the option
 * is bound as `keyBy` instead.
 *
