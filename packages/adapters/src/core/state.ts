...
/** The prop name an adapter binds class names through: `class` in a template, `className` in JSX. */
export const DEFAULT_CLASS_KEY = 'class';

// A class object literal is a fresh object every render, so comparing the raw binding would report
// a change every time; the normalised string is stable and `normalizeClass` is idempotent.
function readProp(props: RiplWritable, key: string, classKey: string): unknown {
    const value = props[key];

    return key === classKey && value !== undefined
 ...
 * @param classKey - The prop name carrying class names. Defaults to `class`.
 * @returns The bound props, with the class binding normalised to a string.
 */
export function readBoundProps(props: RiplWritable, keys: readonly string[], classKey: string = DEFAULT_CLASS_KEY): RiplWritable {
    const output: RiplWritable = {};

    keys.forEach(key => {
        const value = readProp(props, key, classKey);
 ...
 */
export function collectChangedProps(props: RiplWritable, keys: readonly string[], applied: RiplWritable, classKey: string = DEFAULT_CLASS_KEY): RiplWritable | undefined {
 ...
 * @param classKey - The prop name carrying class names. Defaults to `class`.
 */
export function applyFields(element: Element, fields: RiplWritable, writers?: RiplFieldWriters, classKey: string = DEFAULT_CLASS_KEY): void {
