...
- a **CSS selector** or **`HTMLElement`**, in which case the chart creates a Canvas context inside it (the default), or
...
Always `destroy()` the outgoing chart first, since that destroys its scene and context and cleans up all event subscriptions.
