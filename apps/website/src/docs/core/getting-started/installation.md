...
| `@ripl/devtools` | Runtime bridge connecting Ripl contexts, scenes, and renderers to the [Ripl Devtools browser extension](https://chromewebstore.google.com/detail/ripl-devtools/fbcceifmhbcmmbmkphpjepigabdiamjb) |
...
> [!TIP]
> Working in a framework? `@ripl/vue` and `@ripl/react` wrap all of the above as components, each with a `-3d` and a `-charts` companion. See the [Vue docs](/docs/vue/) or the [React docs](/docs/react/).
...
- **Debugging** → `@ripl/devtools` (pairs with the [Ripl Devtools browser extension](https://chromewebstore.google.com/detail/ripl-devtools/fbcceifmhbcmmbmkphpjepigabdiamjb))
